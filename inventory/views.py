...
from .views_pos import (      # ARCH-2 — urls.py hali views.pos_* deb chaqiradi
    QUICK_SELL_CATEGORY,
    _cart_lines, _evaluate_promotions, _open_price_product, _open_price_stock,
    _quick_sell_items, _quick_sell_product, _quick_sell_sync,
    payment_qr_list, payments_webhook,
...
SALES_PER_PAGE = 100   # RPT-1 — bir sahifada nechta qator/chek
...
def sales_list(request):
    ...
    checks_page = None
    if view_mode == 'checks':
        # RPT-1: cheklar ham sahifalanadi (avval eng so'nggi 300 ta edi).
        _txn_qs = (qs.filter(transaction_id__isnull=False)
                   .values('transaction_id')
                   .annotate(_last=Max('sold_at'))
                   .order_by('-_last'))
        checks_page = Paginator(_txn_qs, SALES_PER_PAGE).get_page(page_no)
        _txn_order = list(checks_page.object_list)
    ...
    return render(request, 'inventory/sales_list.html', {
        'view_mode': view_mode,
        'qs_base': qs_base,
        'page_qs': page_qs,        # RPT-1
        'checks': checks,
        'check_count': txn_count,
        'sales': sales,
        # RPT-1 — joriy ko'rinishning sahifasi
        'page_obj': checks_page if view_mode == 'checks' else page_obj,
